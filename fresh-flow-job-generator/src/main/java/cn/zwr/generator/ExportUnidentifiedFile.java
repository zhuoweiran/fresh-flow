 ...
 * 导出可以文件信息
 */
public class ExportUnidentifiedFile {
    public static void main(String[] args) throws Exception {
        Properties properties = new Properties();
        String path = "conf/ExportUnidentifiedFile.properties";
        if(args.length > 0){
            path = args[0];
        }
        ...
        conf.setAppName(ExportUnidentifiedFile.class.getName()).setMaster("local[1]").set("appName", ExportUnidentifiedFile.class.getName());

        System.out.println(conf.get("appName"));

        ESSchemaSource<UnidentifiedFile> ESSchemaSource = new ESSchemaSource<>(conf);
        ...
        UnidentifiedFile unidentifiedFile = new UnidentifiedFile();

        ESSchemaSource.setNodeOptions(options2);
        ESSchemaSource.setT(unidentifiedFile);

        JavaRDD<UnidentifiedFile> helloJavaRDD = ESSchemaSource.read();

        CsvLocalFileSink<UnidentifiedFile> helloCsvLocalFileSink = new CsvLocalFileSink<>();
